...
set_time_limit(0);
/*
function getSceneryLinks ($dir)
{
	$baseLink = '<a href="http://travel.qunar.com/place/city/%s/jingdian-%d">%s</a>';
    $dir = realpath($dir);
    ...
    foreach($dirIterator as $_dir) {
        ...
        if($_dir->isDir()) {
			...
			$i = 1;
			while($i<=84) {
			    error_log(sprintf($baseLink, $dirName, $i, $dirName.'-'.$i) . "\r\n", 3, './scenryLinks.html');
				$i++;
			}
...
getSceneryLinks('./travel.qunar.com/place/city/');
exit;
*/
$server = 'localhost';
$username = 'root';
...
function updateCity ($cityId, $cityInfo)
{
    ...
    if(isset($cityInfo['cityDir'])) {
        $sets[] = " city_dir ='" . mysql_real_escape_string($cityInfo['cityDir']) . "'";
    }
...
function parseScenery ($filepath)
{
    ...
    $breadCrumbs = explode('<span class="e_arrow">></span>', strip_tags($match[0], '<span>'));
    ...
    $return = array('country'=>$country,
                    ...
                    'commInfo'=>$commInfo,
                    'tipInfo'=>$tipInfo,
                    'bigPic'=>(isset($matchBigPic[1]) ? $matchBigPic[1] : array()),
                    'thumbs'=>(isset($matchThumbs[1]) ? $matchThumbs[1] : array()),
                    'percent'=>$percent,
                    'poiId'  => $poiId
    );
    //var_dump($return);
...
//parseQunarCountry('./travel.qunar.com/place/country/');
//parseQunarProvince('./travel.qunar.com/place/province/');
//parseQunarCity('./travel.qunar.com/place/city/');
parseQunarScenery('./travel.qunar.com/place/poi/');
