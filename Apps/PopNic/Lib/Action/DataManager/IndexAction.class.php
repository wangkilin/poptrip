...
class IndexAction extends Action
{
    ...
    protected function _loadlistDefined($prepareData=array())
    {
        ...
        if(isset($listConfig['sql'])) {
            ...
            //echo $listConfig['sql'];exit;
            $fromPos = stripos($listConfig['sql'], ' from ');
            $countSql = 'SELECT COUNT(*) ' . substr($listConfig['sql'], $fromPos);
            import('ORG.Util.Page');// 导入分页类
            $count      = $model->query($countSql);
            //echo $model->getLastSql();
            $count = count($count) > 0 ? count($count) : array_shift($count[0]); // if sql has 'group by', then there is a problem. we need use count() to compute the total number
            $Page       = new Page($count,C('DISPLAY_NUMBER_PERPAGE', 20));// 实例化分页类 传入总记录数和每页显示的记录数
            $show       = $Page->show();// 分页显示输出
            // 进行分页数据查询 注意limit方法的参数要使用Page类的属性
    ...
    protected function _loadFilter($filtersList)
    {
        ...
        if($this->stepIndex==0) {
            $_stepIndex = session($this->tableManageIndex.'/stepIndex');
            $this->stepIndex = $_stepIndex>0 ? $_stepIndex : $this->stepIndex;
        }
        ...
        foreach($steps as $_step) {
            if(is_string($_step)) {
                ...
                $stepData[$_step] = $this->_getFilterStepData($stepData, $_step, $stepInfo);
        ...
        if(($this->stepIndex)<count($filtersList['step'])) {
            ...
            $_POST['tableId'] = I('tableId', null);
            foreach($steps as $_step) {
                if(is_string($_step)) {
                    $_POST[$_step] = $_GET[$_step] = session($this->tableManageIndex.'/'.$_step);
                } else if(is_array($_step)) {
                    foreach($_step as $__step) {
                        $_POST[$__step] = $_GET[$__step] = session($this->tableManageIndex.'/'.$__step);
                    }
                }
            }
        }
        session($this->tableManageIndex.'/stepIndex', $this->stepIndex);
    ...
    protected function _getFilterStepData($stepData, $stepName, $stepInfo)
    {
        ...
        if(isset($stepInfo['sql'])) {
            if(isset($stepInfo['sqlParam'])) {
                ...
                foreach($stepInfo['sqlParam'] as $getKeyName) {
                    $_paramValue = I($getKeyName, null);
                    $_paramValue = $_paramValue===null ? session($this->tableManageIndex.'/'.$getKeyName) : $_paramValue;
                    $paramList[] = $_paramValue;
                    session($this->tableManageIndex.'/'.$getKeyName, $_paramValue);
                }
            ...
            if(isset($stepInfo['columnMap'])) {
                ...
                foreach($stepInfo['columnMap'] as $columnName=>$getKeyName) {
                    ...
                    $_paramValue = $_paramValue===null ? session($this->tableManageIndex.'/'.$getKeyName) : $_paramValue;
                    session($this->tableManageIndex.'/'.$getKeyName, $_paramValue);
                    $where[]= $columnName . "='".addslashes(strval($_paramValue)) . "'";
                }
                $model->where(join(' AND ', $where));
            }
        ...
        if($data && I($stepName, null)!==null) {
            session($this->tableManageIndex.'/'.$stepName, I($stepName, null));
        }
        //echo $model->getLastSql();
