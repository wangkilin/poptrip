 ...
 * @author Walton(line 325 缓存绝对路径做了修改) <zwttmac@gmail.com>
...
class Tpl
{
    ...
    private function __construct()
    {
        $this->_options = array(
            ...
            'suffix' => '.html',  //模板文件后缀	
			'cache_open' => false  //是否开启缓存，程序调试时使用						
        );
    }
    ...
    public function cache($file)
    {
        ...
        //替换循环函数及条件判断语句
		$template = preg_replace("/\{if\s+(.+?)\}/ies","stripvtags('<?php if(\\1) { ?>','')",$template); 
        $template = preg_replace("/\{else\}/ies","stripvtags('<?php } else { ?>','')",$template);   
        $template = preg_replace("/\{elseif\s+(.+?)\}/ies","stripvtags('<?php } elseif (\\1) { ?>','')",$template);   
        $template = preg_replace("/\{\/if\}/ies","stripvtags('<?php } ?>','')",$template);   
        $template = preg_replace("/\{loop\s+(\S+)\s+(\S+)\}/ies","stripvtags('<?php if(is_array(\\1)) foreach(\\1 AS \\2) { ?>','')",$template);   
        $template = preg_replace("/\{loop\s+(\S+)\s+(\S+)\s+(\S+)\}/ies","stripvtags('\n<?php if(is_array(\\1)) foreach(\\1 AS \\2 => \\3) { ?>','')",$template);   
        $template = preg_replace("/\{\/loop\}/ies","stripvtags('\n<?php } ?>\n','')",$template); 
