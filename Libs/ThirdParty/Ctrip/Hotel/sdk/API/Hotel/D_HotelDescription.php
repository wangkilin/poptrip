<?php
/**
 * 请求D_HotelDescription的服务(请求体与D_HotelSearch  一样，根据不同的接口返回不同的数据)
 */
class get_D_HotelDescription{
	/**
	 * @var城市的ID，必须填写
	 */
	var $CityID="";
	...
	private  function getRequestXML()
	{
		...
		$SID=Sid;
		$KEYS=SiteKey;
		$RequestType="D_HotelDescription";
		//构造权限头部
		$headerRight=getRightString($AllianceID,$SID,$KEYS,$RequestType);
	//	$headerRight=getJsonRightString($AllianceID,$SID,$KEYS,$RequestType);
		$city="";
		if($this->CityID!=""){
			$city=<<<BEGIN
...
BEGIN;
			...
			function main(){
				try{
					...
					$commonRequestDo->requestURL=D_HotelDescription_Url;
