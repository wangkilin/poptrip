...
(function (Ctrip) {
    $.ready(function () {
        var cityName = $('#main_Search_CityName');
        var cityId = $('#main_Search_CityID');
        var checkInDate = $('#main_Search_CheckInDate');
        var checkOutDate = $('#main_Search_CheckOutDate');
        var cityPosition = $('#main_Search_Location_Zone');
        ...
        //prepand the data 
        var lzType = $('#barSearch_Location_Type');
        var lzId = $('#barSearch_LZId');
        var oriData = $('#barSearch_Location_Zone').value();
        var tempArr = oriData.split('-');
        if (oriData != ',-,-,') {
            if (tempArr[0] != ',') {
                lzType.value('locationId');
                lzId.value(tempArr[0].split(',')[0]);
            } else if (tempArr[1] != ',') {
                lzType.value('zoneId');
                lzId.value(tempArr[1].split(',')[0]);
            } else {
                lzType.value('district');
            }
        }
        //iframe loader
        Ctrip.iframeLoader = (function () {

            var isOpened = false;
            $('.btn_m_blue').bind('click', function (e) {
                ...
                if (this.alt != '') {
                    ...
                    var url = this.alt;
                    var ifh = wh - 120;
                    var iframeHtml = '<iframe src="' + url + '" width="900px" height="' + ifh + 'px" frameborder="0" id="inneriframe"></iframe>';
                    $('#iframecontent').html(iframeHtml);
                    $('#iframeLoader').css('height', ifh + 'px');
                    $('#iframeLoader').css('margin-top', '-' + ifh / 2 + 'px');
            ...
            $('#closeLoader').bind('click', function (e) {
                $('#iframeLoader').hide();
                $('#mask').hide();
                $('#iframecontent').html('');
        ...
        Ctrip.parseRawData = function () {
            ...
            /*CHINA_HOTEL_METRO_RAW_DATA = CHINA_HOTEL_METRO_RAW_DATA.replace(/@(\d+)\|([1-9]\d*)\|\s*([^\|]+)\|\s*([^\|]*)\|\s*([^\|]*)\|\s*([^@]*)/g, function (_, id, pid, name, pingYing, PY, alias) {
            Ctrip.checkExist(pid, "metro");
            Ctrip.HOTEL_POSITION[pid]["metro"].push({ "display": name, "data": [pingYing, name, id, PY, "metroId"].join("|") });
            Ctrip.HOTEL_POSITION[pid]["all"].push([pingYing, name, id, PY, "metroId"].join("|"));
            return '';
            });
            CHINA_HOTEL_SPOT_RAW_DATA = CHINA_HOTEL_SPOT_RAW_DATA.replace(/@(\w\d+)\|([1-9]\d*)\|\s*([^\|]+)\|\s*([^\|]*)\|\s*([^@]*)/g, function (_, id, pid, name, pingYing, PY, alias) {
            Ctrip.checkExist(pid, "spot");
            Ctrip.HOTEL_POSITION[pid]["spot"].push({ "display": name, "data": [pingYing, name, id, PY, "landMarkId", alias].join("|") });
            Ctrip.HOTEL_POSITION[pid]["all"].push([pingYing, name, id, PY, "landMarkId", alias].join("|"));
            return '';
            });*/
        };
    ...
    Ctrip.checkFrom = function () {
        var cityname = encodeURIComponent(document.main_hotelsearchRquest.main_Search_CityName.value);
        var checkindate = encodeURIComponent(document.main_hotelsearchRquest.main_Search_CheckInDate.value);
        var checkoutdate = encodeURIComponent(document.main_hotelsearchRquest.main_Search_CheckOutDate.value);
        var errorMsg = { pass: true, msg: '' };
        if (cityname === '') {
            errorMsg.pass = false;
            errorMsg.msg = "入住城市不能为空";
            $('#main_Search_CityName')[0].focus();
            return errorMsg;
        }
        if (checkindate === '') {
            errorMsg.pass = false;
            errorMsg.msg = "入住时间不能为空";
            $('#main_Search_CheckInDate')[0].focus();
            return errorMsg;
        }
        if (checkoutdate === '') {
            errorMsg.pass = false;
            errorMsg.msg = "离店时间不能为空";
            $('#main_Search_CheckOutDate')[0].focus();
            return errorMsg;
        }
    ...
    //parse url into a object
    Ctrip.sumbitSearch = function () {
        ...
        var lzid = document.main_hotelsearchRquest.barSearch_LZId.value; //选择器选择后，要做赋值的
        var lztype = document.main_hotelsearchRquest.barSearch_Location_Type.value;
        var lztname = document.main_hotelsearchRquest.main_Search_Location_Zone.value;
        var flzo = "";
        ...
        $('#barSearch_Location_Zone').value(flzo);
        var locationZone = document.main_hotelsearchRquest.barSearch_Location_Zone.value; //区域和商业区
        //var url = Ctrip.siteUrl + "/site/hotelsearch.php?city=" + cityid + "," + cityname;
        var _postUrl = Ctrip.siteUrl + "/site/hotelsearch.php?city=" + cityid + "," + cityname;
        if (star || hotelbrand) {
            _postUrl += "&stb=" + star + ";" + hotelbrand;
        }
        if (hotelname) {
        ...
        if (hotelfacility) {
            _postUrl += "&hf=" + hotelfacility;
        }
        _postUrl += "&pf=1," + pagesize;
		
        var _postParams = "cdate=" + checkindate + "," + checkoutdate + "&stb=" + star + "&hotelbrand=" + hotelbrand + "&price=" + price + "&hname=" + hotelname + "&lzod=" + locationZone + "&hf=" + hotelfacility + "&oy=" + ordername + "," + ordertype + "&pf=" + pagenumber + "," + pagesize;
		if(!_flzoFlag){
			flzo = '';
		}
		if(hotelbrand){
        var _htmlUrl = CtripSelfStrInterchange2("#postDataSearchUrl", cityid + "," + cityname, star+';'+hotelbrand, hotelname, flzo, hotelfacility);
		}
		else{
			var _htmlUrl = CtripSelfStrInterchange2("#postDataSearchUrl", cityid + "," + cityname, star, hotelname, flzo, hotelfacility);
		}
		if(_htmlUrl){
    ...
    repostSearch.bind('click', function (e) {
    ...
    if ($('#toggle_hot').length > 0) {
        Ctrip.hotCities = [{ display: "北京", data: "|北京|1" }, { display: "上海", data: "|上海|2" }, { display: "广州", data: "|广州|32" }, { display: "深圳", data: "|深圳|30" }, { display: "西安", data: "|西安|10" }, { display: "南京", data: "|南京|12" }, { display: "成都", data: "|成都|28" }, { display: "武汉", data: "|武汉|477" }, { display: "天津", data: "|天津|3" }, { display: "重庆", data: "|重庆|4"}];
        var tempHtml = [];
        for (var i = 0; i < Ctrip.hotCities.length; i++) {
            var item = '<li><a class="cityname" href="#" title="' + Ctrip.hotCities[i].data.substr(1) + '">' + Ctrip.hotCities[i].display + '</a></li>';
            tempHtml.push(item);
        }
        $('#toggle_hot').html(tempHtml.join(' '));

        var toggleCitylist = $('.togglelist');
        toggleCitylist.html(Ctrip.defaultCityName);
        toggleCitylist.bind('click', function (e) {
            if ($('#toggle_hot').css('display') == 'none') {
                $('#toggle_hot').css('display', 'block');
            } else {
                $('#toggle_hot').css('display', 'none');
            }
            e.preventDefault();
        })
        $('#toggle_hot').bind('click', function (e) {
            var title = e.target.title;
            var newUrl = Ctrip.rawUrl + '?defaultcityid=' + title.split('|')[1] + ',' + title.split('|')[0];
